...
## from Dynare
function numgrad2(fcn,f0,x,epsilon=1e-6)
...
function csminwel(fcn::Function,
    # grad::Function,
    x0::Vector;
    H0::Matrix           = 1e-4 .* Matrix{eltype(x0)}(I, length(x0), length(x0)),
    rng::AbstractRNG     = MersenneTwister(0),
    verbose::Symbol      = :high,
    x_tol::Real          = 1e-32,  # default from Optim.jl
    f_tol::Float64       = 1e-7, #1e-14,  # Default from csminwel
